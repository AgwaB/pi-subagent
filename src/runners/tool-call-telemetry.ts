...
const SENSITIVE_KEY_PATTERN = /(?:authorization|auth|bearer|cookie|session|token|secret|password|passwd|api[-_]?key|access[-_]?key|refresh[-_]?token|credential|headers?)/i;

type ToolCallStatus = "completed" | "failed" | "incomplete";
type ToolCategory = "network" | "filesystem" | "shell" | "vcs" | "browser" | "agent" | "unknown";

type ToolCallArtifactType = Extract<ArtifactType, "tool-calls" | "tool-calls-summary">;
...
interface AttemptStoreLike {
  writeTextArtifact(type: ToolCallArtifactType, content: string | Uint8Array): Promise<ArtifactRef>;
}
...
function emptyValueSummary(type = "undefined"): ToolValueSummary {
  return { type, stringCount: 0, stringChars: 0, resources: [], truncated: false };
}
...
function sanitizedUrlResource(raw: string): ToolResourceSummary | undefined {
  try {
    ...
    const value = truncate(`${parsed.protocol}//${parsed.host}${path}`, MAX_RESOURCE_LENGTH);
    return { kind: "url", scheme: parsed.protocol.replace(/:$/, ""), host: parsed.host, value };
  } catch {
    return undefined;
  }
}

function collectUrlResources(value: string, resources: ToolResourceSummary[]): void {
  URL_PATTERN.lastIndex = 0;
  for (const match of value.matchAll(URL_PATTERN)) {
    const resource = sanitizedUrlResource(match[0]);
    if (!resource) continue;
    if (resources.some((candidate) => candidate.value === resource.value)) continue;
    resources.push(resource);
    if (resources.length >= MAX_RESOURCES_PER_SUMMARY) return;
  }
}

function mergeSummary(target: ToolValueSummary, nested: ToolValueSummary): void {
  ...
  for (const resource of nested.resources) {
    ...
    if (!target.resources.some((candidate) => candidate.value === resource.value)) target.resources.push(resource);
  }
}

export function summarizeToolValue(value: unknown, depth = 0): ToolValueSummary {
  const type = valueType(value);
  if (value === undefined) return emptyValueSummary("undefined");
  if (value === null || typeof value === "number" || typeof value === "boolean" || typeof value === "bigint") return emptyValueSummary(type);
  ...
  if (Array.isArray(value)) {
    ...
    for (const item of value.slice(0, MAX_KEYS)) mergeSummary(summary, summarizeToolValue(item, depth + 1));
...
function categorizeTool(toolName: string): ToolCategory {
  ...
  if (/^(web_search|fetch_content|fetch|code_search|get_search_content|scrapling_fetch)$/.test(lower) || /(?:web|fetch|http|url|search|scrap)/.test(lower)) return "network";
  if (/^(read|write|edit)$/.test(lower) || /(?:file|fs|path)/.test(lower)) return "filesystem";
  if (/^(bash|shell|exec|command)$/.test(lower) || /(?:shell|terminal|process)/.test(lower)) return "shell";
...
function eventString(record: Record<string, unknown>, field: string): string | undefined {
...
export class ToolCallTelemetryCollector {
  ...
  processEvent(event: unknown, observedAt: Date = new Date()): void {
    ...
    this.addRecord({
      ...
      completedAt,
      durationMs: Math.max(0, observedAt.getTime() - (pending?.startedMs ?? fallback.startedMs)),
      isError,
  ...
  async flush(store: AttemptStoreLike, finishedAt: Date = new Date()): Promise<ArtifactRef[]> {
    ...
    const callsText = this.records.map((record) => JSON.stringify(record)).join("\n") + (this.records.length > 0 ? "\n" : "");
    const summaryText = `${JSON.stringify(this.summary(), null, 2)}\n`;
    const callsRef = await store.writeTextArtifact("tool-calls", callsText);
    const summaryRef = await store.writeTextArtifact("tool-calls-summary", summaryText);
  ...
  private summary(): ToolCallTelemetrySummary {
    ...
    const statusCounts: Record<ToolCallStatus, number> = { completed: 0, failed: 0, incomplete: 0 };
    const urls: string[] = [];
    const hosts: string[] = [];
...
export async function flushToolCallTelemetry(collector: ToolCallTelemetryCollector | undefined, store: AttemptStoreLike): Promise<ArtifactRef[]> {
