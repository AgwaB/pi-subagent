...
try {
  ...
  const failedWait = await waitForSubagent({ cwd, runId: failedStart.runId, taskId: failedStart.taskId, timeoutMs: 1000, pollIntervalMs: 20 });
  assert.equal(failedWait.status, "completed");
  assert.equal(failedWait.snapshot?.status, "failed");
  assert.equal(completionCalls, 1);
  const failedStatus = await getSubagentStatus({ cwd, runId: failedStart.runId, taskId: failedStart.taskId });
