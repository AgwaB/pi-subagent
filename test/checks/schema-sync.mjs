...
const lifecycleKeys = new Set(["action", "runId", "attemptId", "taskId", "pollIntervalMs", "reason", "signal", "escalateAfterMs", "killAfterMs"]);

for (const key of schemaKeys) {
  if (lifecycleKeys.has(key)) continue;
  assert.ok(key in probeValues, `schema key "${key}" missing probe value; add it to this check`);
  const single = key === "tasks" || key === "concurrency"
  ...
  assert.equal(validation.ok, true, `schema key "${key}" must be accepted by validateResolveInput: ${validation.ok ? "" : validation.failure.error}`);
}

// captureToolCalls is reachable end to end: schema, validation, and task items.
assert.ok(schemaKeys.includes("captureToolCalls"), "tool schema exposes captureToolCalls");
const taskSchemaKeys = Object.keys(registeredTool.parameters.properties.tasks.items.properties ?? {});
assert.ok(taskSchemaKeys.includes("captureToolCalls"), "task schema exposes captureToolCalls");
const taskLevel = validateResolveInput({ tasks: [{ agent: "worker", task: "inspect", captureToolCalls: true }] });
assert.equal(taskLevel.ok, true);
assert.equal(taskLevel.input.tasks[0].captureToolCalls, true);

console.log(JSON.stringify({ name: "check-schema-sync", status: "completed", schemaKeys: schemaKeys.length }, null, 2));
